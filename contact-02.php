...
if (isset($_GET['lang'])) {
  $lang = $_GET['lang'];
  if ($lang == "en") {
    $stmt = $conn->prepare("SELECT * FROM contact_02_en");
    $stmt->execute();
    $row_contact_02 = $stmt->fetch(PDO::FETCH_ASSOC);
  } else {
    $stmt = $conn->prepare("SELECT * FROM contact_02");
    $stmt->execute();
    $row_contact_02 = $stmt->fetch(PDO::FETCH_ASSOC);
  }
} else {
  ...
  <form method="POST" action="sendmail02.php">
          <div class="row mt-5">
            ...
            <div class="col-md-12 text-center">
              ...
              <button type="submit" name='sentmail02' class="btn btn btn-dark btn-lg rounded-0 px-lg-5"><i class="icofont-send-mail"></i> ส่งข้อความ</button>
