...
$data_position = $conn->prepare("SELECT * FROM position");
$data_position->execute();
$row_position = $data_position->fetchAll();


$data_detail_pos = $conn->prepare("SELECT * FROM detail_pos ORDER BY id DESC");
...
if (isset($_POST['edit-position'])) {
    ...
    $edit_pos = $conn->prepare("UPDATE position SET position_name = :position_name WHERE id = :id");
...
if (isset($_POST['delete_pos'])) {
    $id = $_POST['delete_pos'];
    $del_pos = $conn->prepare("DELETE FROM position WHERE id = :id");
...
if (isset($_POST['delete_detail_pos'])) {
    ...
    $del_detail_pos = $conn->prepare("DELETE FROM detail_pos WHERE id_pos = :id_pos");
...
if (isset($_POST['save_work_content'])) {
    ...
    $update_work_content = $conn->prepare("UPDATE work_content SET content = :content");
    ...
    if ($update_work_content) {
        ...
        echo "<meta http-equiv='refresh' content='1.5;url=work'>";
    } else {
        echo "<script>
        $(document).ready(function() {
            Swal.fire({
            ...
            <section class="section">
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="flex-end">
                            <a href="work_en"><button type="button" class="btn btn-edit1">EN</button></a>
                                    ...
                                    ?>
                                        <tbody>
                                            ...
                                            <td align="left"><?php echo $row_detail_pos[$i]['content']; ?></td>
                                                ...
                                                <td align="center">
                                                    <div class="manage">
                                                        <a href="detail_pos_edit?work_id=<?php echo $row_detail_pos[$i]['id_pos']; ?>"><button type="button" class="btn" style="background-color:#ffc107; color: #FFFFFF; margin:3px;"><i class="bi bi-pencil-square"></i></button></a>
