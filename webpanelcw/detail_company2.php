    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img['size'] > 0 && $img['error'] == 0) {
            if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                $edit_company = $conn->prepare("UPDATE company2 SET content = :content, img = :img");
                ...
                if ($edit_company) {
                    ...
                    echo "<meta http-equiv='refresh' content='2;url=detail_company2'>";
                } else {
                    echo "<script>
                    $(document).ready(function() {
                        Swal.fire({
        ...
        $edit_company = $conn->prepare("UPDATE company2 SET content = :content ");
        $edit_company->bindParam(":content", $content);
