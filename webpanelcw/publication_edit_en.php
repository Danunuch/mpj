...
if (isset($_GET['publication_id'])) {
    ...
    $publication = $conn->prepare("SELECT * FROM publication_en WHERE id = :id");
    $publication->bindParam(":id", $id);
    $publication->execute();
    $row_publication = $publication->fetch(PDO::FETCH_ASSOC);
}
if (isset($_POST['edit-publi'])) {
    $img = $_FILES['img'];
    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img['size'] > 0 && $img['error'] == 0) {
            if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                $publi = $conn->prepare("UPDATE publication_en SET img = :img, content = :content, link_pdf = :link_pdf WHERE id = :id");

                $publi->bindParam(":img", $fileNew1);
                $publi->bindParam(":content", $content);
                $publi->bindParam(":link_pdf", $link_pdf);
                $publi->bindParam(":id", $id);
                $publi->execute();
                if ($publi) {
                    echo "<script>
        ...
        $publi = $conn->prepare("UPDATE publication_en SET content = :content, link_pdf = :link_pdf WHERE id = :id");
        $publi->bindParam(":content", $content);
        $publi->bindParam(":link_pdf", $link_pdf);
        $publi->bindParam(":id", $id);
        $publi->execute();
                        ...
                        <div class="product-name">
                            <form method="post" enctype="multipart/form-data">
                                <div class="card-header">
                                    ...
                                    <button type="submit" name="edit-publi" class="btn btn-save">Save</button>
                                </div>
                                <textarea name="content"><?php echo $row_publication['content']; ?></textarea>
                                <script>
                                    tinymce.init({
                                        selector: 'textarea',
                                        branding: false,
                                ...
                                <div class="card-body">
                                    <h6>Link</h6>
                                    <input type="text" name="link_pdf" value="<?php echo $row_publication['link_pdf']; ?>" class="form-control">
                                </div>
                                ...
                                <div class="title-img">
                                    ...
                                    <div id="gallery-cover">
                                        <div class='box-edit-img-cover'>
                                            ...
                                            <img class='edit-img-cover' id='previewImg-cover' src="upload/upload_publication/<?php echo $row_publication['img'] ?>">
                                        </div>
                                    </div>
                                </div>
                            </form>
