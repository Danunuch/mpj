    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img['size'] > 0 && $img['error'] == 0) {
            if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                $edit_company = $conn->prepare("UPDATE company1_en SET content = :content, img = :img");
                ...
                if ($edit_company) {
                    ...
                    echo "<meta http-equiv='refresh' content='2;url=detail_company1_en'>";
                } else {
                    echo "<script>
                    $(document).ready(function() {
                        Swal.fire({
        ...
        $edit_company = $conn->prepare("UPDATE company1_en SET content = :content");
        $edit_company->bindParam(":content", $content);
        $edit_company->execute();
        if ($edit_company) {
            echo "<script>
...
if (isset($_POST['edit_blog_company'])) {
    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img['size'] > 0 && $img['error'] == 0) {
            if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                $blog = $conn->prepare("UPDATE blog_company1_en SET img = :img,content = :content WHERE id = :id");
                $blog->bindParam(":img", $fileNew1);
                $blog->bindParam(":content", $content);
        ...
        $blog = $conn->prepare("UPDATE blog_company1_en SET content = :content WHERE id = :id");
        $blog->bindParam(":content", $content);
        $blog->bindParam(":id", $id);
        $blog->execute();
    }
    ...
    if ($blog) {
        ...
        echo "<meta http-equiv='refresh' content='1.5;url=detail_company1_en'>";
    } else {
        echo "<script>
                $(document).ready(function() {
                    Swal.fire({
                ...
                <form method="post" enctype="multipart/form-data">
                    <div class="card">
                        <div class="card-header" style="display: flex ;justify-content: flex-end;">
                            ...
                            <div class="a">
                                <a href="detail_company1"><button type="button" class="btn btn-edit">TH</button></a>
