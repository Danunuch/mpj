...
if (isset($_POST['edit-company'])) {
    ...
    $fileNew1 = rand() . "." . "webp";
    $filePath1 = "upload/upload_company2/" . $fileNew1;

    if (in_array($fileActExt1, $allow)) {
        if ($img['size'] > 0 && $img['error'] == 0) {
            if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                $edit_company = $conn->prepare("UPDATE company2_en SET content = :content, img = :img");
                ...
                if ($edit_company) {
                    ...
                    echo "<meta http-equiv='refresh' content='2;url=detail_company2_en'>";
                } else {
                    echo "<script>
                    $(document).ready(function() {
                        Swal.fire({
        ...
        $edit_company = $conn->prepare("UPDATE company2_en SET content = :content ");
        $edit_company->bindParam(":content", $content);
                ...
                <form method="post" enctype="multipart/form-data">
                    <div class="card">
                        <div class="card-header" style="display: flex ;justify-content: flex-end;">
                            ...
                            <div class="a">
                                <a href="detail_company2"><button type="button" class="btn btn-edit">TH</button></a>
                                <button type="submit" name="edit-company" class="btn btn-save">Save</button>
                            </div>
                        </div>
                        <div class="card-body">
                            <h5>Content</h5>
                            <textarea name="content"><?php echo $row_company2['content']; ?></textarea>
                            <script>
                                tinymce.init({
                                    selector: 'textarea',
                                    branding: false,
