...
if (isset($_GET['publication_id'])) {
    ...
    $publication = $conn->prepare("SELECT * FROM publication WHERE id = :id");
    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img['size'] > 0 && $img['error'] == 0) {
            if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                $publi = $conn->prepare("UPDATE publication SET img = :img, content = :content, link_pdf = :link_pdf WHERE id = :id");
        ...
        $publi = $conn->prepare("UPDATE publication SET content = :content, link_pdf = :link_pdf WHERE id = :id");
        $publi->bindParam(":content", $content);
        $publi->bindParam(":link_pdf", $link_pdf);
        $publi->bindParam(":id", $id);
        $publi->execute();
