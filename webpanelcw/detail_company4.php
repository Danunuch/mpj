    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img['size'] > 0 && $img['error'] == 0) {
            if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                $edit_company = $conn->prepare("UPDATE company4 SET content = :content, img = :img, content1 = :content1, content2 = :content2, content3 = :content3");
                ...
                if ($edit_company) {
                    ...
                    echo "<meta http-equiv='refresh' content='2;url=detail_company4'>";
                } else {
                    echo "<script>
                    $(document).ready(function() {
                        Swal.fire({
        ...
        $edit_company = $conn->prepare("UPDATE company4 SET content = :content, content1 = :content1 ,content2 = :content2 ,content3 = :content3");
        $edit_company->bindParam(":content", $content);
        $edit_company->bindParam(":content1", $content1);
        $edit_company->bindParam(":content2", $content2);
        $edit_company->bindParam(":content3", $content3);
...
if (isset($_POST['edit_blog_company'])) {
    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img['size'] > 0 && $img['error'] == 0) {
            if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                $blog = $conn->prepare("UPDATE blog_company SET img = :img,topic = :topic,content = :content WHERE id = :id");
                $blog->bindParam(":img", $fileNew1);
                $blog->bindParam(":topic", $topic);
                $blog->bindParam(":content", $content);
        ...
        $blog = $conn->prepare("UPDATE blog_company SET topic = :topic,content = :content WHERE id = :id");
        $blog->bindParam(":topic", $topic);
        $blog->bindParam(":content", $content);
        $blog->bindParam(":id", $id);
        $blog->execute();
...
if (isset($_POST['delete_blog'])) {
    ...
    $del_blog = $conn->prepare("DELETE FROM blog_company WHERE id = :id");
                ...
                <form method="post" enctype="multipart/form-data">
                    <div class="card">
                        <div class="card-header" style="display: flex ;justify-content: flex-end;">
                            ...
                            <div class="a">
                                <a href="detail_company4_en"><button type="button" class="btn btn-edit">EN</button></a>
